  ...
  return (
    <li className="element" >
      ...
      <div className="element__like-btn-container">
        <button type="button" className={`element__like ${isLiked && 'element__like_active'}`} aria-label="лайк" onClick={() => props.onCardLike(props.card)}></button>
        <p className="element__like-count">{props.card.likes.length}</p>
      </div>
      {isOwn && <button type="button" className="element__delete" aria-label="удалить" onClick={() => props.onCardDelete(props.card)}></button>}
    </li>
